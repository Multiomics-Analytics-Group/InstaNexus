...
def calculate_nx(seqs, x=50):
    ...
    for length in lengths:
        cum_sum += length
        if cum_sum >= target: return length
    return 0
...
def get_advanced_stats(fasta_path, tool_name, protein_norm, fdr, run_label, category, input_psm_count):
    base_res = {
        "Category": category, "Tool": tool_name, "Sample": run_label, "FDR": fdr,
        "Coverage": 0, "N50": 0, "N90": 0, "Mean_Identity": 0,
        "Spectral_Depth": 0, "Num_Scaffolds": 0, "Max_Length": 0,
        "Precision": 0, "Assembly_Score": 0,
    }

    if not fasta_path or not Path(fasta_path).exists(): return base_res
    seqs = [str(r.seq) for r in SeqIO.parse(fasta_path, "fasta")]
    if not seqs: return base_res
    ...
    if not df_cov.empty:
        ...
        if covered_span > 0: spectral_depth = input_psm_count / covered_span
    ...
    return {
        "Category": category, "Tool": tool_name, "Sample": run_label, "FDR": fdr,
        "Coverage": coverage, "N50": n50, "N90": n90,
        "Mean_Identity": mean_identity, "Spectral_Depth": spectral_depth,
        "Num_Scaffolds": total_scaffolds, "Max_Length": max_hq_len, 
        "Precision": precision, "Assembly_Score": assembly_score,     
    }
...
def prepare_alps_input(df_subset, output_csv_path):
    ...
    if "Peptide" not in df.columns: return False
    df["Peptide"] = df["Peptide"].apply(lambda x: re.sub(r"\(.*?\)", "", str(x)))
    df["Peptide"] = df["Peptide"].apply(lambda x: re.sub(r"\[.*?\]", "", str(x)))
    df["local confidence (%)"] = [" ".join([str(random.randint(85, 99)) for _ in range(len(str(p)))]) for p in df["Peptide"]]
...
def plot_bar_metric(df, metric, y_label, title, output_path, subtitle=""):
    ...
    g = sns.catplot(
        data=df, x="FDR", y=metric, hue="Tool", col="Category", kind="bar",
        palette=colors, height=5, aspect=0.9, legend=False,
        errorbar=("ci", 95), capsize=0.1, err_kws={'linewidth': 1.5, 'color': 'black'} 
    )
    ...
    for ax in g.axes.flat: ax.set_xticklabels([f"{int(x*100)}%" for x in FDR_THRESHOLDS])

    handles = [mpatches.Patch(color=colors["InstaNexus"], label='InstaNexus'),
               mpatches.Patch(color=colors["ALPS"], label='ALPS')]
    g.fig.legend(handles=handles, loc='upper center', bbox_to_anchor=(0.5, 0.92), ncol=2, frameon=False, fontsize=14, title="")
...
def main():
    ...
    with open(METADATA_PATH, "r") as f: FULL_METADATA = json.load(f)

    plot_title_suffix = f"{assembly_mode}, k={kmer_val}"
    if args.refine: plot_title_suffix += " [Refined]"
    ...
    for d in [dir_csv, dir_fasta, dir_svg]: os.makedirs(d, exist_ok=True)

    assembler = Assembler(mode=assembly_mode, kmer_size=kmer_val, min_overlap=3, size_threshold=0, refine_rounds=refine_rounds)
    benchmark_results = []
    ...
    for category, file_list in SAMPLE_GROUPS.items():
        ...
        for filename in file_list:
            ...
            if "experiment_name" in df.columns:
                ...
                df["protease"] = df["experiment_name"].apply(lambda x, p=proteases: preprocessing.extract_protease(x, p))
            
            try: df = robust_clean_dataframe(df)
            except Exception: continue
            if "cleaned_preds" not in df.columns or df.empty: continue
            ...
            for fdr in FDR_THRESHOLDS:
                ...
                try:
                    ...
                    with open(insta_fasta, "w") as f:
                        for i, seq in enumerate(scaffolds): f.write(f">scaffold_{i}\n{seq}\n")
                except: open(insta_fasta, "w").close()

                # ALPS
                alps_input = dir_csv / f"{file_stem}_FDR{fdr}_alps_input.csv"
                prepare_alps_input(subset, alps_input)
                # Cleanup old files
                for f in list(dir_csv.glob(f"{file_stem}_FDR{fdr}*alps*.fasta")): os.remove(f)
                ...
                try:
                    ...
                    if found: alps_raw_fasta = max(found, key=os.path.getctime)
                except: pass
                ...
                for meta in meta_entries:
                    ...
                    benchmark_results.append(get_advanced_stats(insta_fasta, "InstaNexus", p_norm, fdr, lbl, category, len(subset)))
                    benchmark_results.append(get_advanced_stats(alps_raw_fasta, "ALPS", p_norm, fdr, lbl, category, len(subset)))
