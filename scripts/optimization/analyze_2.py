...
def classify_sequence_type(row):
    ...
    if row.get("refine_rounds", 0) == 0: return "Contigs"
    else: return "Scaffolds"
...
def plot_grid_search_clustermap(df, index_cols, column_cols, theme, value_col, title="", aggfunc="max", output_file=None):
    ...
    fig = go.Figure(data=[go.Heatmap(
        z=pivot.values, x=col_labels, y=row_labels, colorscale=theme,
        zmin=0, zmax=1, showscale=True, colorbar=dict(title=value_col, len=0.75, thickness=20)
    )])
    
    n_rows, n_cols = pivot.shape
    shapes = []
    for i in range(n_rows + 1): shapes.append(dict(type="line", x0=-0.5, x1=n_cols-0.5, y0=i-0.5, y1=i-0.5, line=dict(color="white", width=2)))
    for j in range(n_cols + 1): shapes.append(dict(type="line", x0=j-0.5, x1=j-0.5, y0=-0.5, y1=n_rows-0.5, line=dict(color="white", width=2)))

    fig.update_layout(
        width=950, height=850, title=dict(text=title, x=0.5),
        xaxis=dict(tickangle=-45, showgrid=False, zeroline=False, title=", ".join([ABBREV.get(c,c) for c in column_cols])),
        yaxis=dict(showgrid=False, zeroline=False, autorange="reversed", title=", ".join([ABBREV.get(c,c) for c in index_cols])),
        plot_bgcolor="white", paper_bgcolor="white", shapes=shapes, font=dict(family="Arial", size=14)
    )
    if output_file:
...
def compute_and_add_scaled_metrics(df):
    ...
    if not existing_metrics: return df
...
def save_detailed_rankings(df_best, output_dir, mode_name):
    ...
    for cat, sub in df_best.groupby("category"):
        ...
        desired_order = [
            "category", "sample", "assembly_method", "source", 
            "total_sequences", "total_sequences_scaled", 
            "N50", "N50_scaled", 
            "coverage", "coverage_scaled", 
            "composite_score"
...
def main():
    ...
    for f in all_files:
        ...
        except Exception as e: print(f"Skipped {f}: {e}")
    ...
    for _, row in unique_combinations.iterrows():
        subset = full_df[(full_df["category"] == row["category"]) & 
