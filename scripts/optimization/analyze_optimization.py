...
def normalize_composite_score(df):
    if "composite_score" not in df.columns: return df
...
def save_detailed_rankings(df_best, output_dir, mode_name):
    for cat, sub in df_best.groupby("category"):
        ...
        desired_order = ["category", "sample", "assembly_method", "total_sequences", "N50", "coverage", "mean_identity", "composite_score"]
...
def main():
    ...
    for _, row in unique_combinations.iterrows():
        ...
        for metric in ["composite_score", "coverage"]:
            ...
            plot_grid_search_clustermap(subset, valid_y, valid_x, theme_colors, metric, title, output_file=out_subdir/fname)
