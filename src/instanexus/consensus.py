...
def plot_heatmap(pssm_df, output_file):
    ...
    cmap = sns.cubehelix_palette(start=2, rot=0, dark=0.15, light=0.85, reverse=True, as_cmap=True)
...
def plot_logo(pssm_df, output_file):
    fig, ax = plt.subplots(figsize=[min(len(pssm_df) / 1.5, 50), 3])
    logo = logomaker.Logo(
        pssm_df,
        ax=ax,
    ...
    logo.style_xticks(anchor=0, rotation=0, spacing=1, fontsize=14, ha="center")
...
def process_alignment_files(align_folder: str, output_folder: str, run_id: str = ""):
    """
    Process all alignment (.afa) files directly inside the alignment folder
    and generate consensus sequences, heatmaps, and logos.
    """
    ...
    for alignment_file in tqdm(
        sorted(os.listdir(align_path)),
        desc=f"[{run_id}] Consensus" if run_id else "Consensus",
    ):
        if not alignment_file.endswith(".afa"):
            continue
    ...
    print("All consensus tasks completed.")

def generate_consensus_stats(consensus_base_folder):
    fasta_files = [f for f in os.listdir(consensus_base_folder) if f.endswith("_consensus.fasta")]
    if not fasta_files:
        return
    lengths = []
    gap_lengths_all = []
    ...
    for fasta_file in fasta_files:
        fasta_path = os.path.join(consensus_base_folder, fasta_file)
    ...
    stats_path = os.path.join(consensus_base_folder, "consensus_stats.json")
...
def load_all_consensus_sequences(consensus_base_folder):
    consensus_dict = {}
    fasta_files = [f for f in os.listdir(consensus_base_folder) if f.endswith((".fasta", ".fa"))]
    for filename in fasta_files:
        filepath = os.path.join(consensus_base_folder, filename)
        try:
            for record in SeqIO.parse(filepath, "fasta"):
                consensus_dict.setdefault("sequences", []).append(str(record.seq))
